...
if (isset($_GET["status"])) {
	if($_GET["status"]==1){ //Drafted
		...
		WHERE A.application_status = '1' AND A.active_status = '1' AND region_id = '$user_region'";
		$result = $conn->query($sql);
	}else if($_GET["status"]==2){ //Requested
		$sql = "SELECT A.id, A.deceased_name, A.deceased_ic, A.application_status, R.name
		FROM fbp_application A LEFT JOIN fbp_region R ON A.region_id = R.id
		WHERE A.application_status = '2' AND A.active_status = '1' AND region_id = '$user_region'";
		$result = $conn->query($sql);
	}else if($_GET["status"]==3){ //Verified
		$sql = "SELECT A.id, A.deceased_name, A.deceased_ic, A.application_status, R.name
		FROM fbp_application A LEFT JOIN fbp_region R ON A.region_id = R.id
		WHERE A.application_status = '3' AND A.active_status = '1' AND region_id = '$user_region'";
		$result = $conn->query($sql);
	}else if($_GET["status"]==4){ //Etiqa
		$sql = "SELECT A.id, A.deceased_name, A.deceased_ic, A.application_status, R.name
		FROM fbp_application A LEFT JOIN fbp_region R ON A.region_id = R.id
		WHERE A.application_status = '4' AND A.active_status = '1' AND region_id = '$user_region'";
		$result = $conn->query($sql);
	}else if($_GET["status"]==5){ //Completed
		$sql = "SELECT A.id, A.deceased_name, A.deceased_ic, A.application_status, R.name
		FROM fbp_application A LEFT JOIN fbp_region R ON A.region_id = R.id
		WHERE A.application_status = '5' AND A.active_status = '1' AND region_id = '$user_region'";
		$result = $conn->query($sql);
	}else if($_GET["status"]==0){ //Rejected
		$sql = "SELECT A.id, A.deceased_name, A.deceased_ic, A.application_status, R.name
		FROM fbp_application A LEFT JOIN fbp_region R ON A.region_id = R.id
		WHERE A.application_status = '0' AND A.active_status = '1' AND region_id = '$user_region'";
		$result = $conn->query($sql);
	}
}else{
	$sql = "SELECT A.id, A.deceased_name, A.deceased_ic, A.application_status, R.name
	FROM fbp_application A LEFT JOIN fbp_region R ON A.region_id = R.id
	WHERE A.active_status = '1' AND region_id = '$user_region'";
	$result = $conn->query($sql);
}
